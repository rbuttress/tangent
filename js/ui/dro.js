...
export class DRO {
    ...
    attachEvents() {
        ...
        c.querySelectorAll('.jog-btn[data-axis]').forEach(btn => {
            btn.onmousedown = (e) => this.handleJog(btn.dataset.axis, parseInt(btn.dataset.dir), e);
        });
...
handleJog(axis, dir, event) {
    ...
    // 2. CRITICAL: Pull currentPos from the SHARED machine object
    // If machine.currentPos[axis] is 0 because of a logic error, 
    // it will always move to 0 + (dir * step)
    const current = machine.currentPos[axis];
    const target = current + (dir * step);

    const port = localStorage.getItem('last-port');
    if (port) {
        const cmd = `G90 G0 ${axis.toUpperCase()}${target.toFixed(4)}`;
        this.spjs.send(`send ${port} ${cmd}\n`);
        
        // Use connection.logToConsole for visual feedback of what was sent
        console.log(`UI JOG: ${axis.toUpperCase()} from ${current} to ${target}`);
    }
}
    ...
    homeAxis(axis) {
        const port = localStorage.getItem('last-port');
        if (port) this.spjs.send(`send ${port} G28.2 ${axis.toUpperCase()}0\n`);
    }

    homeAll() {
        const port = localStorage.getItem('last-port');
        if (!port) return;
        // Logic for Z -> A -> X -> Y sequence
        const axes = ['z', 'a', 'x', 'y'];
        axes.forEach(ax => {
            const conf = machine.config.axes[ax];
            if (conf && (conf.sn > 0 || conf.sx > 0)) {
                this.spjs.send(`send ${port} G28.2 ${ax.toUpperCase()}0\n`);
            }
        });
    }
